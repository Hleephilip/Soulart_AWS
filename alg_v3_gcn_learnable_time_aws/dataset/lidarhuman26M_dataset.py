...
Auxiliary functions
'''
def cluster_and_filter_point_cloud(points, std_multiplier=2.0, min_size=4, k=8, gap_threshold=20):
    """
    ...
    """
    N = points.shape[0]
    z_values = points[:, 0]  # Extract Z values (depth)

    # Step 1: Build KD-Tree for nearest neighbor search
    tree = KDTree(points[:, 1:])

    # Step 2: Identify connected components using BFS with KD-Tree
    ...
    for i in range(N):
        if not visited[i]:
            ...
            while queue:
                ...
                _, neighbors = tree.query(points[cur, 1:], k=k)
                for neighbor in neighbors:
                    if neighbor != cur and not visited[neighbor]:
                        visited[neighbor] = True
                        queue.append(neighbor)

            components.append(component)

    # Step 3: Compute statistics for each component
    ...
    for comp in components:
        ...
        z_comp = z_values[comp]
        mean = np.mean(z_comp)
        std_dev = np.std(z_comp)

        lower_bound = mean - std_multiplier * std_dev
        upper_bound = mean + std_multiplier * std_dev

        filtered_comp = [idx for idx in comp if lower_bound <= z_values[idx] <= upper_bound]

        if len(filtered_comp) >= min_size:
            filtered_components.append(filtered_comp)
    
    # print('\nfiltered:', filtered_components)
    # Step 4: Identify reference group
    reference_index = -1
    reference_size = 0
    for c, comp in enumerate(filtered_components):
        if len(comp) > reference_size:
            reference_size = len(comp)
            reference_index = c

    if reference_index == -1:
        return np.empty((0, 3))  # No valid clusters

    # Step 5: Define Z boundaries for filtering
    ref_comp = filtered_components[reference_index]

    # print('\nref:', ref_comp)
    z_ref = z_values[ref_comp]
    ref_min_z = np.min(z_ref)
    ref_max_z = np.max(z_ref)
    # print(ref_min_z, ref_max_z)
    
    n = (ref_max_z - ref_min_z) // 2
    lower_z_bound = max(0, ref_min_z - n)
    upper_z_bound = ref_max_z + n

    # Step 6: Check reachability of other groups
    dist = np.full(N, -1, dtype=float)
    queue = deque(ref_comp)
    for idx in ref_comp:
        dist[idx] = 0

    reachable_components = set([reference_index])
    ...
    while queue:
        ...
        _, neighbors = tree.query(points[cur, 1:], k=k)
        for neighbor in neighbors:
            if neighbor != cur and dist[neighbor] == -1:
                z = z_values[neighbor]

                if current_dist + 1 <= gap_threshold:
                    dist[neighbor] = current_dist + 1
                    queue.append(neighbor)

                    for c, comp in enumerate(filtered_components):
                        if c not in reachable_components and neighbor in comp:
                            z_comp = z_values[comp]
                            if np.any((z_comp >= lower_z_bound) & (z_comp <= upper_z_bound)):
                                reachable_components.add(c)

    # Step 7: Collect final filtered points
    final_points = []
    for c, comp in enumerate(filtered_components):
        if c in reachable_components:
            final_points.extend(points[comp])
    # print('reachable:', reachable_components)
...
class lidarhuman26MDataset(Dataset):
    ...
    def _prepare_data(self):
        ...
        with zipfile.ZipFile(os.path.join(self.root_dir, f'{self.mode}.zip'), mode='r') as z:
            for _, _, frame_paths in tqdm(self.data):
                for p in frame_paths:
                    if p not in self.plys:
                        p_format = p.split(self.mode)[1][1:]
                        with z.open(p_format, 'r') as plyfile:
                            _ply = read_point_cloud_ply(plyfile)
                            self.plys[p] = _ply
        return 
    ...
    def __getitem__(self, idx):
        ...
        xs = self.joint_pos_abs[sbj][curr_frame:curr_frame+self.T] # T, J, 3
        xs = xs - xs[:, 0:1, :] # make the root-relative joint coordinate
        rs = self.joint_rot[sbj][curr_frame:curr_frame+self.T] # T, J, 6
        x = self.joint_pos_abs[sbj][curr_frame+self.T-1] # J, 3
        x = x - x[0:1, :]
        r = self.joint_rot[sbj][curr_frame+self.T-1] # J, 6
        abs_x = self.joint_pos_abs[sbj][curr_frame+self.T-1, 0] # 3
        ...
        for i, t in enumerate(range(curr_frame, curr_frame + self.T)):
            ...
            points = self.plys[path]
            points = cluster_and_filter_point_cloud(points)
...
def main():
    ...
    # Save to .ply file
    o3d.io.write_point_cloud('j_ld26_recon_move.ply', point_cloud)
    # all float32
