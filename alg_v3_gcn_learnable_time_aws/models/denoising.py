...
class Denoiser(nn.Module):
    def __init__(self, config):
        ...
        self.time_prior_chans = config.time_emb_chans // 4
        self.time_emb_chans = config.time_emb_chans
        ...
        self.time_embed = nn.Sequential(
            nn.Linear(self.time_prior_chans, self.time_emb_chans),
            nn.SiLU(),
        ...
        _last_chans = self.imu_chans
        for out_channel in self.mlp:
            self.mlp_ffns.append(nn.Linear(_last_chans + 3 + self.time_emb_chans, out_channel)) # 6 stands for timestep (3) + joint position embedding (3)
            if self.use_gcn:
                self.graphs_a.append(nn.Parameter(torch.randn(1, out_channel, self.n_joints, self.n_joints), requires_grad = True))
                self.graphs_w.append(nn.Sequential(
                        nn.Conv1d(out_channel, out_channel, kernel_size = 1, stride = 1, padding = 0, bias = True),
    ...
    def timestep_embedding(self, timesteps, dim, max_period=10000):
        ...
        if dim % 2:
            embedding = torch.cat([embedding, th.zeros_like(embedding[:, :1])], dim=-1)
        return embedding
    ...
    def forward(self, joints_t, t, cond_k):
        ...
        time_emb = self.time_embed(self.timestep_embedding(t, self.time_prior_chans)).repeat(1, J, 1)
