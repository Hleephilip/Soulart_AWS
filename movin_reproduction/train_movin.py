...
def train_step(net, dataloader, criterion, epoch, optimizer, report_interval, logging, normalized):
    ...
    for i, item in enumerate(dataloader):
        ...
        if i % report_interval == 0:
            log_text = f'Epoch {epoch} Step {i} Train loss {loss.item():.4f}  MPJPE {loss_MPJPE.item()*100:.2f}cm  ang {loss_ang.item():.2f}deg'
...
def val_step(net, dataloader, criterion, epoch, report_interval, logging, normalized):
    ...
    with torch.no_grad():
        ...
        for i, item in enumerate(dataloader):
            ...
            # print loss info
            if i % report_interval == 0:
                log_text = f'Epoch {epoch} Step {i} Validation loss {loss.item():.4f}  MPJPE {loss_MPJPE.item()*100:.2f}cm  ang {loss_ang.item():.2f}deg'
                print_log(log_text, logging)
...
def train(cfg):
    ...
    for epoch in range(start_epoch, cfg.exp.epochs):
        ...
        log_text = f"[Epoch {epoch}] Train loss {log['loss']:.4f}  MPJPE {log['mpjpe']*100:.4f}cm  ABS {log['abs']*100:.4f}cm  ang {log['ang']:.4f}deg"
        print_log(log_text, logging)
        ...
        log_text = f"[Epoch {epoch}] Validation loss {log['loss']:.4f}  MPJPE {log['mpjpe']*100:.4f}cm  \
