...
if [ $VOLUME_ID ]; then
		...
		# Get training code
		git clone https://github.com/awslabs/Soulart_AWS.git
		chown -R ubuntu: Soulart_AWS
		cd Soulart_AWS/

		source ~/.bashrc
		conda env create -f pointbert.yaml
		...
		cd ../movin_reproduction/
		# Initiate training using the tensorflow_36 conda environment
		sudo -H -u ubuntu bash -c "source /home/ubuntu/anaconda3/bin/activate pointbert; python train_movin.py --config ./cfgs/lidarhuman26M.yaml"
fi
