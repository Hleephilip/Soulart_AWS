...
class LIPDDataset(torch.utils.data.Dataset):
    def __init__(self, cfgs):
        ...
        self.dataset = []
        self.use_rel_coodinate = False # default
    ...
    def __getitem__(self, index):
        ...
        for i, example in enumerate(example_seq):
            ...
            pc_data = pc_data - cetroid
            pc_data = farthest_point_sample(pc_data, self.num_points) # (self.n_points, 3)
            pc_data = pc_data + cetroid
            ...
            seq_j.append(torch.from_numpy(gt_j.reshape(-1, 3))) # 24, 3
        ...
        xs = torch.stack(seq_j, axis=0).to(torch.float32) # T, J, 3
        rs = torch.stack(seq_gt, axis=0).to(torch.float32) # T, J, 6
        xs = xs - xs[:, 0:1, :] # make the root-relative joint coordinate
        x = seq_j[-1].to(torch.float32) # J, 3
        x = x - x[0:1, :]
        r = seq_gt[-1].to(torch.float32) # J, 6
        ...
        point_clouds = point_clouds - seq_cetroid.unsqueeze(0).unsqueeze(0)
        Item = {
            'pts': point_clouds,
            'xs': xs,
            'rs': rs,
...
def main():
    ...
    cfgs = load_config("../cfgs/dataset_cfgs/LIPD.yaml")
    ...
    scale = item['scale']
    print(scale.shape)
    print(pc.dtype, xs.dtype, rs.dtype, cetroid.dtype, scale.dtype)
    # all float32
    # print(xs[-1])
    # print(x)
    print(r.shape)
    ...
    point_cloud.points = o3d.utility.Vector3dVector(x) # j_vis
    # point_cloud.points = o3d.utility.Vector3dVector(pc[-1]) # pc_vis
    # Save to .ply file
